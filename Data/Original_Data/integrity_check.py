...
def _integrity_checks() -> None:
    fhr = pd.read_csv(
        f"{paths.ORIGINAL_DATA_PATH}/fhr_ctu-chb.csv", index_col=0, compression="gzip"
    )
    uc = pd.read_csv(
        f"{paths.ORIGINAL_DATA_PATH}/uc_ctu-chb.csv", index_col=0, compression="gzip"
    )
    clinical = pd.read_csv(
        f"{paths.ORIGINAL_DATA_PATH}/clinical_ctu-chb.csv",
        index_col=0,
        compression="gzip",
    )
    ...
    clinical_pickle = pd.read_pickle(
        f"{paths.ORIGINAL_DATA_PATH}/clinical_ctu-chb.pickle"
    )
