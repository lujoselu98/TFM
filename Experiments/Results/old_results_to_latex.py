...
from Utils import fixed_values, latex, pandas_utils, paths
...
def create_latex_table(
    ...
    dataset: str,
    metric: str,
    mark_best_preprocess: Optional[bool] = True,
    mark_best_classifier: Optional[bool] = False,
    style_mark: Optional[str] = "textbf",
) -> str:
    """
    ...
    for classifier in classifiers:
        ...
        for preprocess in preprocesses:
            row.append(
                f"{classifier_data[('mean', preprocess)]:.3f} "
                f"{latex.PM_STRING} "
    ...
    for row in table:
        value_means = [
            float(value.split(f" {latex.PM_STRING} ")[0]) for value in row[1:]
        ]
    ...
    table_lines.append(
        ...
        + dataset
        + "} "
        + fixed_values.EVALUATION_METRICS[metric]["name"]
        + " "
        + dataset
        + "}"
    )
    ...
    if mark_best_preprocess:
        for best_preprocess_value in best_preprocess:
            latex_table = latex_table.replace(
                best_preprocess_value, f"\\{style_mark}{{{best_preprocess_value}}}"
            )

    if mark_best_classifier:
        for best_classifier_value in best_classifier:
            latex_table = latex_table.replace(
                best_classifier_value, f"\\{style_mark}{{{best_classifier_value}}}"
            )
...
def compose_latex(file: str, metric: str) -> str:
    ...
    for dataset in fixed_values.DATASETS:
        whole_latex += "\n".join(
            [
                "\t\t" + line
                for line in create_latex_table(file, dataset, metric).split("\n")
            ]
        )
...
def main() -> None:
    ...
    for metric in fixed_values.EVALUATION_METRICS.keys():
        latex.save_latex(
            compose_latex(
                f"{paths.RESULTS_PATH}/17_02_22_results_main_experiment.csv", metric
            ),
