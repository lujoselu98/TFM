...
def parallel_param_validation(
    ...
    y_train: pd.Series,
    y_test: pd.Series,
    clf_to_val: sklearn.base.ClassifierMixin,
    preprocess: str,
    features_number: int,
    params: dict,
) -> float:
    ...
    if preprocess == "whole":
        ...
        X_train_pre_f, X_test_pre_f = preprocessing.get_features(
            X_train, X_test, features_number
        )
...
def main_experiment(
    ...
    for dataset in progress_bar:
        # if dataset != 'FFT':
        #     continue
        _, X, y = common_functions.load_data(
            dataset,
                ...
                for idx_external in range(EXTERNAL_SPLITS):
                    ...
                    for idx_internal in range(fixed_values.INTERNAL_SPLITS):
                        ...
                        if preprocess == "whole":
                            ...
                            X_train_pre, X_test_pre = preprocessing.load_preprocess(
                                dataset,
                                preprocess,
                                idx_external,
                                idx_internal,
                        ...
                        for params in param_permutations:
                            ...
                            internal_results = joblib.Parallel(n_jobs=5)(
                                joblib.delayed(parallel_param_validation)(
                                    X_train_pre_save[idx_internal],
                                    X_test_pre_save[idx_internal],
                                    y_train_save[idx_internal],
                                    y_test_save[idx_internal],
                                    clf_to_val,
                                    preprocess,
                                    features_number,
                                    params,
                    ...
                    if preprocess == "whole":
                        ...
                        X_train_pre_f, X_test_pre_f = preprocessing.get_features(
                            X_train_pre, X_test_pre, best_features_number
                        )
                    ...
                    csv_string = (
                        f"{dataset};{preprocess};{classifier_name};{idx_external};"
...
def smoothed_data_experiment() -> None:
    """
    Function to made all experiments on smoothed data
    """

    progress_bar = tqdm(
        fixed_values.FilterSet,
        total=len(fixed_values.FilterSet)
        * len(fixed_values.CLASSIFIERS)
        * (len(fixed_values.PREPROCESSES) + 1)
        * fixed_values.EXTERNAL_SPLITS_SHUFFLE,
    )
        ...
        for classifier_name, classifier in fixed_values.CLASSIFIERS.items():
            ...
            for preprocess in fixed_values.PREPROCESSES + ["whole"]:
                ...
                for idx_external in range(fixed_values.EXTERNAL_SPLITS_SHUFFLE):
                    tqdm_desc = (
                        f"Filter set: {filter_set.name} "
                        f"({(list(fixed_values.FilterSet)).index(filter_set) + 1}"
                        f"/{len(fixed_values.FilterSet)}) "
                        f"Clf: {classifier_name} "
                        f"({(list(fixed_values.CLASSIFIERS.keys())).index(classifier_name) + 1}"
                        f"/{len(fixed_values.CLASSIFIERS)}) "
                        f"Pre: {preprocess} "
                    ...
                    for idx_internal in range(fixed_values.INTERNAL_SPLITS):
                        (
                            ...
                            y_train,
                            y_test,
                        ) = common_functions.load_smoothed_data(
                            idx_external,
                            idx_internal,
                            filter_data=filter_data,
                            easy_data=easy_data,
                        ...
                        if preprocess == "whole":
                            ...
                            (
                                X_train_pre,
                                X_test_pre,
                            ) = preprocessing.smoothed_load_preprocess(
                                preprocess,
                                idx_external,
                                idx_internal,
                                filter_data=filter_data,
                    ...
                    csv_string = (
                        f"DFT_{filter_set.name};{preprocess};{classifier_name};{idx_external};"
                        f"{best_features_number};{best_params};{metrics_dict}"
                    )
...
def dummy_classifier(
    ...
    progress_bar = tqdm(
        fixed_values.DATASETS, total=len(fixed_values.DATASETS) * EXTERNAL_SPLITS
    )
    ...
    for dataset in progress_bar:
        ...
        for idx_external in range(EXTERNAL_SPLITS):
            ...
            csv_string = (
                f"{dataset};{None};{classifier_name};{idx_external};"
...
if __name__ == "__main__":
    ...
    main_experiment(
        strategy="randomsplit", remove_outliers=False, filter_data=True, easy_data=False
    )
