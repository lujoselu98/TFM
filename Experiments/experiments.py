...
def smoothed_data_experiment() -> None:
    ...
    progress_bar = tqdm(fixed_values.FilterSet,
                        total=len(fixed_values.FilterSet) * len(fixed_values.CLASSIFIERS) *
                ...
                for idx_external in range(fixed_values.EXTERNAL_SPLITS_SHUFFLE):
                    ...
                    for idx_internal in range(fixed_values.INTERNAL_SPLITS):
                        ...
                        if preprocess == 'whole':
                            ...
                            X_train_pre, X_test_pre = preprocessing.smoothed_load_preprocess(preprocess,
                                                                                             idx_external, idx_internal,
                                                                                             filter_data=filter_data,
                                                                                             easy_data=easy_data)
                        X_train_pre_save.append(X_train_pre)
                    ...
                    csv_string = f"DFT_{filter_set.name};{preprocess};{classifier_name};{idx_external};" \
                                 f"{best_features_number};{best_params};{metrics_dict}"
