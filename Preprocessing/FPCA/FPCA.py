...
def save_FPCA(
    ...
    for idx_external in tqdm(range(EXTERNAL_SPLITS)):
        ...
        X_train_pca, X_test_pca = calculate_FPCA(
            X_train, X_test, tt, n_components=fixed_values.MAX_DIMENSION
        )

        components_file = (
            f"{save_path}/{filter_path}{easy_path}{dataset}_PCA_{idx_external}"
        )
