...
def save_PLS(
    ...
    tt, X, y = common_functions.load_data(
        dataset, remove_outliers, filter_data, easy_data
    )
    ...
    for idx_external in tqdm(range(EXTERNAL_SPLITS)):
        ...
        X_train_pls, X_test_pls = calculate_PLS(
            X_train, y_train, X_test, n_components=fixed_values.MAX_DIMENSION
        )

        components_file = (
            f"{save_path}/{filter_path}{easy_path}{dataset}_PLS_{idx_external}"
        )
        ...
        for idx_internal in range(fixed_values.INTERNAL_SPLITS):
            ...
            X_train_pca, X_test_pca = calculate_PLS(
                X_train, y_train, X_test, n_components=fixed_values.MAX_DIMENSION
            )
...
def smoothed_save_PLS(
    ...
    for idx_external in tqdm(range(fixed_values.EXTERNAL_SPLITS_SHUFFLE)):
        ...
        for idx_internal in range(fixed_values.INTERNAL_SPLITS):
            tt, X_train, X_test, y_train, y_test = common_functions.load_smoothed_data(
                idx_external, idx_internal, filter_data, easy_data
            )
