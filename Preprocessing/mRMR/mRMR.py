...
def calculate_mRMR(
    ...
    for idx in candidates:
        ...
        relevance_vector[idx] = dcor.distance_correlation(
            row_vals, y_train, method=dcor.DistanceCovarianceMethod.AVL
        )
    ...
    while len(selected_features_index) < features_number:
        ...
        for idx in candidates:
            ...
            accumulated_redundancy[idx] += dcor.distance_correlation(
                actual_vals, last_vals, method=dcor.DistanceCovarianceMethod.MERGESORT
            )
...
def parallel_external_code(
    ...
    selected_features_index = calculate_mRMR_skfda(
        X_train, tt, y_train, features_number=fixed_values.MAX_DIMENSION
    )

    return selected_features_index


def smoothed_parallel_external_code(
    idx_external: int,
    ...
    selected_features_index = calculate_mRMR_skfda(
        X_train, pd.Series(tt), y_train, features_number=fixed_values.MAX_DIMENSION
    )

    return selected_features_index


def parallel_internal_code(
    X: pd.DataFrame,
    tt: pd.Series,
    y: pd.Series,
    idx_external: int,
...
def smoothed_parallel_internal_code(
    idx_external: int,
    idx_internal: int,
    filter_data: Optional[bool] = False,
    easy_data: Optional[bool] = False,
    ...
    """
    tt, X_train, X_test, y_train, y_test = common_functions.load_smoothed_data(
        idx_external, idx_internal, filter_data=filter_data, easy_data=easy_data
    )
...
def save_mRMR_indexes(
    ...
    tt, X, y = common_functions.load_data(
        dataset,
    ...
    selected_features_indexes_ext = joblib.Parallel(n_jobs=8)(
        joblib.delayed(parallel_external_code)(
            X, tt, y, idx_external, strategy, remove_outliers_dataset
        )
    ...
    for i, idx_external in tqdm(
        enumerate(range(EXTERNAL_SPLITS)),
        desc=f"Saving .txt {dataset}",
        ...
        selected_features_indexes_int = joblib.Parallel(n_jobs=8)(
            joblib.delayed(parallel_internal_code)(
                X, tt, y, idx_external, idx_internal, strategy, remove_outliers_dataset
            )
...
def smoothed_save_mRMR_indexes(filter_data, easy_data) -> None:
    ...
    selected_features_indexes_ext = joblib.Parallel(n_jobs=8)(
        joblib.delayed(smoothed_parallel_external_code)(
            idx_external, filter_data=filter_data, easy_data=easy_data
        )
        for idx_external in tqdm(
            range(fixed_values.EXTERNAL_SPLITS_SHUFFLE), desc=f"mRMR"
        )
    )
    ...
    for i, idx_external in tqdm(
        enumerate(range(fixed_values.EXTERNAL_SPLITS_SHUFFLE)),
        desc=f"Saving .txt",
        ...
        selected_features_indexes_int = joblib.Parallel(n_jobs=8)(
            joblib.delayed(smoothed_parallel_internal_code)(
                idx_external, idx_internal, filter_data=filter_data, easy_data=easy_data
            )
            for idx_internal in range(fixed_values.INTERNAL_SPLITS)
        )
        ...
        for j, idx_internal in enumerate(range(fixed_values.INTERNAL_SPLITS)):
            ...
            sel_features_file = (
                f"{folder_path}/sel_features_{idx_external}_{idx_internal}.txt"
            )
            with open(sel_features_file, "w") as f:
                f.write(str(selected_features_index.tolist()))


def load_mRMR_indexes(
    dataset: str,
    idx_external: int,
...
def smoothed_load_mRMR_indexes(
    idx_external: int,
    idx_internal: Optional[int] = None,
    filter_data: Optional[bool] = False,
    easy_data: Optional[bool] = False,
...
def save_mRMR(
    ...
    for idx_external in tqdm(range(EXTERNAL_SPLITS), desc=f"Saving .pickle {dataset}"):
        ...
        pickle_file = (
            f"{save_path}/{filter_path}{easy_path}{dataset}_mRMR_{idx_external}"
        )
        ...
        for idx_internal in range(fixed_values.INTERNAL_SPLITS):
            X_train, X_test, y_train, y_test = common_functions.get_fold(
                X, y, idx_external, idx_internal, strategy=strategy
            )

            mRMR_indexes = load_mRMR_indexes(
                dataset, idx_external, idx_internal, remove_outliers=remove_outliers
            )
...
def smoothed_save_mRMR(
    ...
    for idx_external in tqdm(
        range(fixed_values.EXTERNAL_SPLITS_SHUFFLE), desc=f"Saving .pickle"
    ):
        tt, X_train, X_test, y_train, y_test = common_functions.load_smoothed_data(
            idx_external, filter_data=filter_data, easy_data=easy_data
        )
        ...
        for idx_internal in range(fixed_values.INTERNAL_SPLITS):
            ...
            mRMR_indexes = smoothed_load_mRMR_indexes(
                idx_external, idx_internal, filter_data=filter_data, easy_data=easy_data
            )
            mRMR_indexes = list(map(str, mRMR_indexes))
