...
def load_smoothed_data(
    ...
    internal_cv = sklearn.model_selection.StratifiedKFold(
    ...
    return (
        ...
        X_train.iloc[index_test],
        y_train.iloc[index_train],
        y_train.iloc[index_test],
    )
...
def get_fold(
    ...
    if strategy == "kfold":
        external_cv = sklearn.model_selection.StratifiedKFold(
            n_splits=fixed_values.EXTERNAL_SPLITS, shuffle=True, random_state=0
        )
    else:
        external_cv = sklearn.model_selection.StratifiedShuffleSplit(
            n_splits=fixed_values.EXTERNAL_SPLITS_SHUFFLE,
    ...
    if idx_internal is not None:
        ...
        internal_cv = sklearn.model_selection.StratifiedKFold(
            n_splits=fixed_values.INTERNAL_SPLITS,
            shuffle=True,
        ...
        return (
            X_int.iloc[index_train],
            X_int.iloc[index_test],
            y_int.iloc[index_train],
            y_int.iloc[index_test],
        )
...
def filter_patterns(
    ...
    check_list = [
        x
        for x in before_dismissed
        if (x not in fhr_now_dismissed) and (x not in uc_now_dismissed)
    ]
