...
def plot_data_desc(
    ...
    X: pd.DataFrame,
    y: pd.Series,
    title: Optional[str] = "Dataset description",
    y_label: Optional[str] = "dataset value",
    ...
    ax.fill_between(
        tt, min_0, max_0, label="Class 0 Min./Max.", alpha=0.25, color=COLORS[0]
    )
    ...
    ax.fill_between(
        tt, min_1, max_1, label="Class 1 Min./Max.", alpha=0.25, color=COLORS[1]
    )
...
def plot_fhr_uc(
    ...
    ax[0].plot(
        fhr[y == 0].mean().index, fhr[y == 0].mean().values, label="Media FHR normal"
    )
    ax[0].plot(
        fhr[y == 1].mean().index,
        fhr[y == 1].mean().values,
        label="Media FHR patológico",
    )
    ax[1].plot(
        uc[y == 0].mean().index, uc[y == 0].mean().values, label="Media UC normal"
    )
    ax[1].plot(
        uc[y == 1].mean().index, uc[y == 1].mean().values, label="Mean UC patológico"
    )
...
def plot_class_proportion(
    ...
    sns.barplot(
        x="index",
        y="ph",
        data=pd.DataFrame(y.value_counts(normalize=True)).reset_index(),
        ax=ax,
    )
...
def plot_relevance(
    ...
    ax.plot(
        np.array(list(relevance_vector.keys()), dtype="float64") / 4,
        relevance_vector.values(),
    )
...
def plot_CD_diagram(
    results_file: str, dataset: str, width: Optional[int] = 10
) -> plt.Figure:
    """
    ...
    lowv, highv = (
        0,
        dataset_results.MODEL_NAME.unique().__len__(),
    )

    for idx_external in range(ranks):
        ordered_results = dataset_results[
            dataset_results["IDX_EXTERNAL"] == idx_external
    ...
    fig = evaluation.graph_ranks(
        list(models_avg_rank.values()),
        names=list(models_avg_rank.keys()),
        width=width,
        lowv=lowv,
        highv=highv,
        cd=cd,
...
def plot_results(csv_file: str, metric: str, extra: Optional[str] = ""):
    ...
    ax = sns.stripplot(
        data=stripplot_data, y=metric, x="DATASET", hue="MODEL_NAME", dodge=True
    )
    box = ax.get_position()
